...
Access control is **not** a fixed set of role capabilities anymore - it's a per-employee, Owner-editable checklist of 8 permission flags:

```
viewDashboard, viewSalesHistory, usePOS,
manageProducts, manageInventory, manageCustomers, manageSuppliers, manageEmployees
```

**The Owner always has full access, no matter what.** Only a paid registration (see section 6) creates an Owner account, so ultimately every permission anyone else has traces back to something the paying Owner explicitly chose to hand out.

When the Owner (or a Manager with `manageEmployees`) creates an employee, picking a Role (Manager/Cashier/Inventory Staff) just **pre-fills a sensible starting checklist** - every single box stays fully editable, both at creation and later via "Edit":

| Role starting template | Granted by default |
|---|---|
| **Manager** | View Dashboard, View Sales History, Manage Products, Manage Inventory, Manage Customers, Manage Suppliers, Manage Employees |
| **Cashier** | View Dashboard, View Sales History, Use POS Terminal, Manage Inventory |
| **Inventory Staff** | View Dashboard, View Sales History, Manage Inventory |

Want a Cashier who can also manage Suppliers? Check that one box. Want a Manager who can't touch Employees? Uncheck that one box. It's entirely up to the Owner.

**Enforced in two places, with the backend as the real boundary:**
1. **Frontend** (`dashboard.html` + `app.js`): nav items carry `data-permission="..."` instead of a role list; `applyPermissionVisibility()` shows/hides each one based on the logged-in employee's actual `permissions` object.
2. **Backend** (`middleware/auth.js`'s `requirePermission(key)`): every sensitive route checks the real permission flag stored in MongoDB - the frontend hiding a nav link is just a courtesy, the API call itself is what's actually gated.

**Privilege-escalation guard:** nobody can hand out more than they themselves have. If a Manager doesn't have `manageSuppliers`, they can never grant it to a Cashier they create - not through the UI, and not by tampering with the request body either, since `backend/utils/permissions.js`'s `sanitizePermissions()` caps every request against the granter's own permission set before it's ever saved.
