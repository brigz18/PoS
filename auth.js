...
// Kept for the handful of things that are truly role-based rather than
// permission-based (e.g. only an Owner can edit business/billing settings).
...
// Restricts a route to accounts holding a specific granular permission, e.g.
// requirePermission('manageProducts'). An Owner always passes, regardless of
// their `permissions` object - see utils/permissions.js for the full model.
// Every module in the app (Products, Inventory, Customers, Suppliers,
// Employees, POS, Sales History, Dashboard) is gated this way instead of by
// a fixed role, so the Owner can grant/revoke access per employee.
const requirePermission = (permissionKey) => (req, res, next) => {
  ...
  const granted = req.user.permissions && req.user.permissions[permissionKey] === true;
  if (!granted) {
    res.status(403);
    throw new Error(`Your account does not have the '${permissionKey}' permission. Ask your business Owner to grant it in Employees.`);
  }
  next();
};
